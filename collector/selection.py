# 분봉·호가 수집 대상을 상위군과 무작위 대조군으로 선정한다. 생존 편향을 막기 위함이다 (D-032)
...
def select_candidates(
    ...
    전 종목 분봉은 API 호출 제한상 불가능해서 선정이 필요하다.
    그런데 오른 종목만 모으면 분모가 없어, 에이전트가 반드시 생존 편향에 빠진
    규칙을 만들어낸다. 그래서 대조군 0을 허용하지 않는다 (D-032).
